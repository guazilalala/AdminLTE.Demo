...
function loadTables(startPage, pageSize) {
    ...
    $.ajax({
        type: "GET",
        url: "/DataDictionary/GetAllPageList?startPage=" + startPage + "&pageSize=" + pageSize + "&_t=" + new Date().getTime(),
        success: function (data) {
            $.each(data.rows, function (i, item) {
                var tr = "<tr>";
                tr += "<td align='center'><input type='checkbox' class='checkboxs' value='" + item.id + "'/></td>";
                tr += "<td>" + (item.parentId == "00000000-0000-0000-0000-000000000000" ? "根字典" : item.parentId) + "</td>";
                tr += "<td>" + item.name + "</td>";
